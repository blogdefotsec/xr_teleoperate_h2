...
from unitree_sdkr2py.core.channel import ChannelPublisher, ChannelSubscriber, ChannelFactoryInitialize # dds
...
class H2_ArmController:
    def __init__(self, simulation_mode = False):
        ...
        logger_mp.debug(f"Current all body motor state q:\n{self.all_motor_q} \n")
        logger_mp.debug(f"Current two arms motor state q:\n{self.get_current_dual_arm_q()}\n")
        logger_mp.info("Lock all joints except two arms to URDF default (0.0)...")
        ...
        for id in H2_JointIndex:
            ...
            if id.value in arm_indices:
                if self._Is_wrist_motor(id):
                    ...
                    self.msg.motor_cmd[id].kp = self.kp_low
                    self.msg.motor_cmd[id].kd = self.kd_low
                # Arm joints follow user input later, but set initial to real state or 0
                self.msg.motor_cmd[id].q = self.all_motor_q[id]
            else:
                if self._Is_weak_motor(id):
                    self.msg.motor_cmd[id].kp = self.kp_low
                ...
                # Lock non-arm joints to URDF zero position instead of current read position
